...
CREATE TABLE IF NOT EXISTS conversation_logs (
  ...
  note TEXT,
  round_label TEXT,
  ai_score_overall INTEGER DEFAULT 0 CHECK (ai_score_overall >= 0 AND ai_score_overall <= 100),
  ai_score_relevance INTEGER DEFAULT 0 CHECK (ai_score_relevance >= 0 AND ai_score_relevance <= 100),
  ai_score_weird INTEGER DEFAULT 0 CHECK (ai_score_weird >= 0 AND ai_score_weird <= 100),
  ai_score_offtrack INTEGER DEFAULT 0 CHECK (ai_score_offtrack >= 0 AND ai_score_offtrack <= 100),
  ai_analysis TEXT,
  created_at TIMESTAMP DEFAULT NOW(),
